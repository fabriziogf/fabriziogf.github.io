...
6. **CI.** Find the run for this commit with `gh run list --branch main --limit 5 --json databaseId,headSha,status -q ".[] | select(.headSha==\"$(git rev-parse HEAD)\")"` (it can take a few seconds to appear; the installed `gh` has no `--commit` flag), then `gh run watch <id> --exit-status`. Report the actual conclusion.
